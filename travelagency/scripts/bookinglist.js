window.onload = function(){
    ...
    function onClick(e){
        ...
        for (var i = 0, length = radios.length; i < length; i++) {
            if (radios[i].checked) {
                ...
                if(e.target.id === "search"){
                    xhr.open('GET', 'booking.php?booking='+response+"&option="+option+"&context=search", true);
                }else if (e.target.id === "sort"){
                    xhr.open('GET', 'booking.php?booking='+response+"&option="+option+"&context=sort", true);
                }else if (e.target.id === "des"){
                    xhr.open('GET', 'booking.php?booking='+response+"&option="+option+"&context=des", true);
                }
                xhr.send();
                return;
            }
        ...
        if(e.target.id === "search" && response === ""){
            xhr.open('GET', 'booking.php?booking='+response+"&context=search"+"&option", true);
